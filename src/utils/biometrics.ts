...
export class Biometrics {
  ...
  public static async encryptPassphraseAndSave(
    password: string,
    biometricPromptMessage: string,
    startSaving?: () => Promise<boolean>,
    ...
    try {
      ...
      const signResult = await ReactNativeBiometrics.createSignature({
        payload: 'Password',
        promptMessage: biometricPromptMessage,
        cancelButtonText: localization.component_button.cancel,
      });
      if (signResult.success) {
        ...
        const storage: AppStorage = new AppStorage();
        const encryptedPassword: string = encryption.encrypt(
          '0123456789' + password,
  ...
  public static async clearPassphraseFromStorage(
  ...
  public static async getPassphrase(
    promptMessage: string,
  ): Promise<{success: boolean; password?: string; error?: string}> {
    try {
      const storage: AppStorage = new AppStorage();
      const encryptedPassword: string = await storage.getItem(
        AppStorage.ENCRYPTED_PASSWORD,
      );
      if (!encryptedPassword) {
        return {
          success: false,
          error: localization.errors.error_biometric_disabled,
        };
      }
      const signResult = await ReactNativeBiometrics.createSignature({
        payload: 'Password',
      ...
      if (signResult.success) {
        const password: string = encryption.decrypt(
          encryptedPassword,
          signResult.signature,
        );
